...
select count(categoria) from farmacia where categoria = 'blue'or categoria = 'black';

-- 6 - Faça uma query que retorna todas as categorias não nulas e a soma do estoque de todos os 
...
-- medicamento separado por um hífen e a categoria entre parenteses de todos os registros da 
-- tabela `farmacia`. Quando a categoria for nula, substituir por `(sem categoria)`. 
...
select nome, idade, cadastro from usuarios where cadastro ilike '2020%'
...
-- que retorne apenas a quantidade de anos, meses, dias em um objeto. 
